...
class Session(object):
	...
	def enter_enable_mode(self):
		...
		if self.session_state == Session.PRIVILEDGE_MODE:
			return True
		raise Exception("Trying to enter enable mode while State is not Availble or already in priviledge mode")
		return False
	...
	def enter_configuration_mode(self):
		...
		if self.session_state == Session.PRIVILEDGE_MODE:
			...
			raise Exception("Attempted to enter configuration mode when device was not in priviledge mode on '%s'" % self.hostname)
	...
	def exit_configuration_mode(self):
		...
		if self.session_state == Session.CONFIGURATION_MODE:
			...
			raise Exception("Attempted to exit configuration mode when device was not in configuration mode on '%s'" % self.hostname)
	...
	def create_crypto_keys(self, keytype='rsa', modulus=2048):
		...
		if self.session_state == Session.CONFIGURATION_MODE:
			...
			raise Exception("Attempted to configuration crypto keys when device was not in configuration mode on '%s'" % self.hostname)
	...
	def set_hostname(self,hostname):
		...
		if self.session_state == Session.CONFIGURATION_MODE:
			...
			raise Exception("Attempted to configuration hostname while device was not in configuration mode on '%s'" % self.hostname)
	...
	def upgrade_code_by_tftp(self,tftp_server,filename,towhere):
		...
		if self.session_state == Session.PRIVILEDGE_MODE:
			...
			i = self.session.expect(['Done.','Error','please wait',pexpect.TIMEOUT],timeout=300)
			if i==1:
			...
			return True
		raise Exception("Attempted to upgrade %s code while device was not in priviledge mode on '%s'" % (towhere,self.hostname))
		return False
	...
	def reload(self, writemem=True):
		...
		if self.session_state == Session.PRIVILEDGE_MODE:
			...
			logging.warning("Attempted to logout when device was not priviledge mode on '%s'" % self.hostname)
