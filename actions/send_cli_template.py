...
class SendCLITemplateAction(actions.SessionAction):
    ...
    def run(self, via, device, template, template_dir='', variables='{}', conf_mode=False, username='', password='', enable_username='', enable_password=''):
	...
	has_failures=False
	for d in devices:
        	session = ztp_utils.start_session(d, self._username, self._password, self._enable_username, self._enable_password, via)
